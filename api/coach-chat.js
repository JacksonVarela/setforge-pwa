...
export default function CoachChat() {
  ...
  async function handleSend(text) {
    ...
    try {
      ...
      console.warn("[CoachChat] falling back:", e?.message || e);
      const fallback =
        !online
...
function sanitizeForAPI(msgs) {
  // Keep it lean for your API — only what’s needed
  return msgs.map(({ role, content }) => ({ role, content }));
}
function defaultAssistant(userContent) {
  // Light fallback if /api/coach-chat isn’t present
