...
export default function CoachChat({ units="lb" }) {
  const [messages, setMessages] = useLocalState("sf.chat", [
    { role:"assistant", text:"Yo. I’m Ronin — hypertrophy coach. Ask me anything about training, programming, or nutrition." }
  ]);
  const [input, setInput] = useState("");
  const [busy, setBusy] = useState(false);
  const scrollerRef = useRef(null);

  useEffect(()=>{ if(scrollerRef.current){ scrollerRef.current.scrollTop = scrollerRef.current.scrollHeight; }}, [messages,busy]);

  async function send() {
    const text = input.trim(); if(!text||busy) return;
    setInput(""); setMessages(m=>[...m,{role:"user",text}]); setBusy(true);
    try {
      const reply = await coachChatSend([{role:"user",content:text}], { units, day:"" });
      setMessages(m=>[...m,{role:"assistant",text:reply||"(no reply)"}]);
    } catch {
      setMessages(m=>[...m,{role:"assistant",text:"Couldn’t reach AI. Check API key or network."}]);
    } finally { setBusy(false); }
  }

  function ask(q){ setInput(q); setTimeout(send,0); }
    ...
    <section className="rounded-2xl border border-neutral-800 p-0 overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-neutral-800 bg-neutral-900">
        ...
        <div className="hidden sm:flex gap-2">
          <button className="pill" onClick={()=>ask("Give me a 4-day UL split focused on chest and hamstrings.")}>4-day UL focus</button>
          <button className="pill" onClick={()=>ask("How should I progress dumbbell bench if I hit failure at 12 reps?")}>Progression tip</button>
          <button className="pill" onClick={()=>ask("Give me three cues to improve my barbell row for lats.")}>Form cues</button>
        </div>
      </div>
      ...
      <div ref={scrollerRef} className="h-[60vh] overflow-y-auto px-3 py-3 space-y-2">
        {messages.map((m,i)=>(
          <div key={i} className={m.role==="user"?"flex justify-end":"flex justify-start"}>
            <div className={"max-w-[85%] px-3 py-2 rounded-xl border "+(m.role==="user"?"bg-white text-neutral-900 border-white":"bg-neutral-900 text-neutral-100 border-neutral-800")}>
              {m.text}
            </div>
          </div>
        ))}
      ...
      <div className="p-3 border-t border-neutral-800 bg-neutral-900">
        <div className="flex items-center gap-2">
          <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} className="input flex-1" placeholder="Ask Ronin about training, programming, or diet…" />
